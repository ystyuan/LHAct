...
class BasicBlock(nn.Module):
    ...
    def forward_threshold(self, x, threshold=1e10):
        ...
        b, c, w, h = out.shape
        mask = out.view(b, c, -1).mean(2) < threshold #在basic block和bottleneck里，使用mask将高于阈值的激活置0
...
class Bottleneck(nn.Module):
    ...
    def forward_threshold(self, x, threshold=1e10):
        ...
        out = self.relu(out)
        b, c, w, h = out.shape
        mask = out.view(b, c, -1).mean(2) < threshold  #观察mask,mean(2)
...
class ResNet(AbstractResNet):
    ...
    def forward_threshold(self, x, threshold_h,threshold_l):
        x = self.maxpool(self.relu(self.bn1(self.conv1(x))))
        x= self.layer4(self.layer3(self.layer2(self.layer1(x))))
        x = self.avgpool(x)
